  ...
  module LessConversion
    ...
    def process_stylesheet_assets
      ...
      files.each do |name, file|
        ...
        if name.start_with?('bootstrap/mixins/')
        ...
        case name
          when 'bootstrap/mixins/buttons.less', 'square/buttons.less'
            file = replace_all file, /(\.dropdown-toggle)&/, '&\1'
          when 'bootstrap/mixins/list-group.less'
            file = replace_rules(file, '  .list-group-item-') { |rule| extract_nested_rule rule, 'a&' }
          when 'bootstrap/mixins/gradients.less'
            file = replace_ms_filters(file)
            file = deinterpolate_vararg_mixins(file)
          when 'bootstrap/mixins/vendor-prefixes.less'
          ...
          when 'bootstrap/mixins/grid-framework.less'
            file = convert_grid_mixins file
          when 'bootstrap/component-animations.less'
            file = extract_nested_rule file, "#{SELECTOR_RE}&\\.in"
          when 'bootstrap/responsive-utilities.less'
            file = apply_mixin_parent_selector file, '\.(?:visible|hidden)'
          when 'bootstrap/variables.less'
            ...
            file = replace_all file, %r{(\$icon-font-path): \s*"(.*)" (!default);}, "\n" + unindent(<<-SCSS, 14)
              ...
              \\1: if($bootstrap-sass-asset-helper, "sq-bootstrap/", "\\2sq-bootstrap/") \\3;
            SCSS
          when 'square/variables.less'
            ...
            file = replace_all file, %r{(\$sq-icons-font-path): \s*"(.*)";}, "\n" + unindent(<<-SCSS, 14)
              // [converter] If $bootstrap-sass-asset-helper if used, provide path relative to the assets load path.
              // [converter] This is because some asset helpers, such as Sprockets, do not work with file-relative paths.
              \\1: if($bootstrap-sass-asset-helper, "sq-bootstrap/", "\\2sq-bootstrap/") \\3;
            SCSS

            file = replace_all file, %r{(\$sq-market-font-path): \s*"(.*)";}, "\n" + unindent(<<-SCSS, 14)
              // [converter] If $bootstrap-sass-asset-helper if used, provide path relative to the assets load path.
              // [converter] This is because some asset helpers, such as Sprockets, do not work with file-relative paths.
              \\1: if($bootstrap-sass-asset-helper, "sq-bootstrap/", "\\2sq-bootstrap/") \\3;
            SCSS
          when 'bootstrap/close.less'
            # extract .close { button& {...} } rule
            file = extract_nested_rule file, 'button&'
          when 'bootstrap/dropdowns.less'
          ...
          when 'bootstrap/navbar.less'
            file = replace_all file, /(\s*)\.navbar-(right|left)\s*\{\s*@extend\s*\.pull-(right|left);\s*/, "\\1.navbar-\\2 {\\1  float: \\2 !important;\\1"
          when 'bootstrap/tables.less'
            file = replace_all file, /(@include\s*table-row-variant\()(\w+)/, "\\1'\\2'"
          when 'bootstrap/thumbnails.less', 'bootstrap/labels.less', 'bootstrap/badges.less', 'bootstrap/buttons.less'
          ...
          when 'sq-icons.less'
            file = replace_rules(file, /\s*@font-face/) { |rule|
              rule = replace_all rule, /(\$sq-icons-font(?:-\w+)+)/, '#{\1}'
              replace_asset_url rule, :font
            }
          when 'square/fonts.less'
            file = replace_rules(file, /\s*@font-face/) { |rule|
              rule = replace_all rule, /(\$sq-market-font-(?:path|name))/, '#{\1}'
              replace_asset_url rule, :font
            }
          when 'bootstrap/type.less'
      ...
      # move sq-bootstrap/_sq-bootstrap.scss to _sq-bootstrap.scss adjusting import paths
      main_from = "#{save_to}/_sq-bootstrap.scss"
      main_to   = File.expand_path("#{save_to}/../_sq-bootstrap.scss")
      content = File.read(main_from).gsub(/ "/, ' "sq-bootstrap/')
      content = content.prepend("@import \"sq-bootstrap/square/variables\";\n\n")
      save_file main_to, content
